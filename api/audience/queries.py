...
def get_audience_ratings(id):
    with get_db() as conn:
        with conn.cursor() as cursor:
            ...
            if rating_data:
                return {"title": rating_data[0], "average_rating": round(rating_data[1], 1), "num_ratings": rating_data[2], "poster": rating_data[3]}
            return None
...
def get_similar_highly_rated_movies_same_genre(id):
    with get_db() as conn:
        with conn.cursor() as cursor:
            ...
            SELECT m.id, m.title, m.poster, m.average_rating, m.num_ratings
            FROM movies m
            ...
            similar_movies_same_genre_high = [{'id': movie[0], 'title': movie[1], 'poster': movie[2], 'average_rating': round(movie[3], 1), 'num_ratings': movie[4]} for movie in similar_movies_same_genre_high]
...
def get_similar_lowly_rated_movies_same_genre(id):
    with get_db() as conn:
        with conn.cursor() as cursor:
            ...
            similar_movies_same_genre_low = [{'id': movie[0], 'title': movie[1], 'poster': movie[2], 'average_rating': movie[3], 'num_ratings': movie[4]} for movie in similar_movies_same_genre_low]
...
def get_similar_highly_rated_movies_different_genre(id):
    with get_db() as conn:
        with conn.cursor() as cursor:
            ...
            SELECT DISTINCT m.id, m.title, m.poster, m.average_rating, m.num_ratings, g.name
            FROM movies m
            JOIN other_high_rated_movies ohrm ON m.id = ohrm.movieId
            JOIN movie_genres mg ON mg.movie_id = m.id
            ...
            similar_movies_diff_genre_high = [{'id': movie[0], 'title': movie[1], 'poster': movie[2], 'average_rating': round(movie[3], 1), 'num_ratings': movie[4], 'genre': movie[5]} for movie in similar_movies_diff_genre_high]
...
def get_similar_lowly_rated_movies_different_genre(id):
    with get_db() as conn:
        with conn.cursor() as cursor:
            ...
            similar_movies_diff_genre_low = [{'id': movie[0], 'title': movie[1], 'poster': movie[2], 'average_rating': movie[3], 'num_ratings': movie[4], 'genre': movie[5]} for movie in similar_movies_diff_genre_low]
...
def get_movie_genre(id):
    with get_db() as conn:
        with conn.cursor() as cursor:
            ...
            return [{"genre": genre[0]} for genre in genres]
    ...
    with get_db() as conn:
        with conn.cursor() as cursor:
            ...
            genre_high_ratings AS (
                SELECT g.genre_name, COUNT(hrm.rating) AS high_rating_count
            ...
            genre_high = [{'genre_name': genre[0]} for genre in genre_high]
    ...
    with get_db() as conn:
        with conn.cursor() as cursor:
            ...
            genre_low_ratings AS (
                SELECT g.genre_name, COUNT(lrm.rating) AS low_rating_count
                FROM low_rated_movies lrm
                JOIN movie_genres mg ON r.movieId = mg.movie_id
                JOIN genres g ON mg.genre_id = g.id
                WHERE mg.genre_id NOT IN (SELECT genre_id FROM input_movie_genre)
                GROUP BY g.genre_name
            ...
            genre_low = [{'genre_name': genre[0]} for genre in genre_low]
